...
describe('App', () => {
  ...
  it('TC-068: limit slider affects search results', async () => {
    ...
    mockAxios.onGet('/search').reply((config) => {
      const k = parseInt(config.params?.k || '10', 10);
    ...
    await waitFor(() => {
      const searchCall = mockAxios.history.get.find((call) => call.url === '/search');
      expect(searchCall?.params?.k).toBe('25');
    });
  });
