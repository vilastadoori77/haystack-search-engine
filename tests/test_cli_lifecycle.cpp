...
static int run_command_capture_output(const std::string &cmd, std::string &stdout_output, std::string &stderr_output)
{
    ...
    std::string stdout_cmd = cmd + " 1>/tmp/haystack_stdout_$$ 2>/tmp/haystack_stderr_$$; echo $?";
    int result = std::system(stdout_cmd.c_str());
    
    // Read captured output
    std::ifstream stdout_file("/tmp/haystack_stdout_$$");
    std::ifstream stderr_file("/tmp/haystack_stderr_$$");
    ...
    // Cleanup
    std::system("rm -f /tmp/haystack_stdout_$$ /tmp/haystack_stderr_$$");
...
TEST_CASE("Nonexistent index directory, exit code 3")
{
    ...
    std::string searchd_path = find_searchd_path();
    std::string cmd = searchd_path + " --serve --in \"" + nonexistent_dir + "\" --port 8900 2>&1";
