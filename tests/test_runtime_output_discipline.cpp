...
TEST_CASE("Output discipline: startup success messages appear only on stdout")
{
    ...
    pid_t pid = getpid();
    std::string stdout_file_path = "/tmp/haystack_output_stdout_" + std::to_string(pid);
    std::string stderr_file_path = "/tmp/haystack_output_stderr_" + std::to_string(pid);
    
    std::string cmd = searchd_path + " --serve --in \"" + index_dir + "\" --port " + std::to_string(test_port);
    std::string full_cmd = "(" + cmd + " >" + stdout_file_path + " 2>" + stderr_file_path + " &); SERVE_PID=$!; sleep 0.5; kill $SERVE_PID 2>/dev/null || true; wait $SERVE_PID 2>/dev/null; exit 0";
    std::system(full_cmd.c_str());
